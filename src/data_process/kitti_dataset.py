...
class KittiDataset(Dataset):
    ...
    def remove_invalid_idx(self, image_idx_list):
        ...
        for sample_id in image_idx_list:
            ...
            # --- NEW FILTERING LOGIC ---
            # Filter raw objects to remove classes that cause mathematical instability (nan loss)
            # --- UPDATED FILTERING LOGIC ---
            filtered_objects = []
            for obj in objects:
                # 1. Skip problematic classes
                if obj.type == 'DontCare':
                    continue
                
                # 2. Skip boxes with zero or negative dimensions (The "nan" Fix)
                if obj.l <= 0.1 or obj.w <= 0.1 or obj.h <= 0.1:
                    continue
            ...
            if not noObjectLabels:
                # Convert rect camera coordinates to lidar coordinates
                labels[:, 1:] = transformation.camera_to_lidar_box(labels[:, 1:], calib.V2C, calib.R0, calib.P)

            valid_list = []
            for i in range(labels.shape[0]):
                # Double-check against your config allowed classes
                if int(labels[i, 0]) in cnf.CLASS_NAME_TO_ID.values():
                    # Ensure the object is physically within your BEV boundary
                    if self.check_point_cloud_range(labels[i, 1:4]):
                        valid_list.append(labels[i, 0])

            # Only add the sample to the training list if it contains at least one valid object
            if len(valid_list) > 0:
